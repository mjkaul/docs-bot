...
canonical_path: /en/latest/serve/monitoring
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/monitoring.md
...
> **Tip:** To learn more about the Serve controller actor, the HTTP proxy actor(s), the deployment replicas, and how they all work together, check out the [Serve Architecture](serve-architecture) documentation.
...
> **Note:** Log messages include the logging level, timestamp, deployment name, replica tag, request ID, route, file name, and line number.
...
> **Note:** Access log is Ray Serve traffic log, it is printed to proxy log files and replica log files per request. Sometimes it is useful for debugging, but it can also be noisy.
...
> **Note:** The request ID is used to associate logs across the system. Avoid sending
...
> **Note:** Different metrics are collected when deployments are called via Python `DeploymentHandle` versus HTTP/gRPC.
...
> **Warning:** **Histogram bucket configuration**
...
> **Note:** These lifecycle **histograms** use `deployment` and `application` labels only—no `replica` label—so Prometheus cardinality stays manageable at scale.
