---
id: serve.advanced-guides.grpc-guide
title: Set Up a gRPC Service
topic_type: task
description: ''
subjects:
- serve
...
- runtime-env
references:
- /en/latest/serve/api/doc/ray.serve.grpc_util.RayServegRPCContext
canonical_path: /en/latest/serve/advanced-guides/grpc-guide
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/advanced-guides/grpc-guide.md
...
> **Note:** Ensure that the generated files are in the same directory as where the Ray cluster
...
> **Note:** The default max gRPC message size is ~2GB. To adjust it, set `RAY_SERVE_GRPC_MAX_MESSAGE_SIZE` (in bytes) before starting Ray, e.g., `export RAY_SERVE_GRPC_MAX_MESSAGE_SIZE=104857600` for 100MB.
...
> **Note:** `route_prefix` is still a required field as of Ray 2.7.0 due to a shared code path with
...
> **Note:** Serve provides the `RayServeAPIServiceStub` stub, and `HealthzRequest` and
...
> **Note:** At this point, two applications are running on Serve, "app1" and "app2". If more
...
> **Note:** If the handler raises an unhandled exception without setting a status code on the
