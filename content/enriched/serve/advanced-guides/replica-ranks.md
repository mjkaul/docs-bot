---
id: serve.advanced-guides.replica-ranks
title: Replica ranks
topic_type: task
description: ''
subjects:
...
- replica
references:
- /en/latest/serve/api/doc/ray.serve.context.ReplicaContext
- /en/latest/serve/api/doc/ray.serve.get_replica_context
- /en/latest/serve/api/doc/ray.serve.schema.ReplicaRank
canonical_path: /en/latest/serve/advanced-guides/replica-ranks
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/advanced-guides/replica-ranks.md
license: Apache 2.0, The Ray Authors
---

# Replica ranks

> **Warning:** This API is experimental and may change between Ray minor versions.
...
> **Note:** **Requirements to receive rank updates:**
...
> **Tip:** If you'd like different behavior for when `reconfigure` is called with rank changes, [open a GitHub issue](https://github.com/ray-project/ray/issues/new/choose) to discuss your use case with the Ray Serve team.

## How replica ranks work

> **Note:** **Rank reassignment is eventually consistent**
...
> **Note:** **Ranks don't influence scheduling or eviction decisions**
...
> **Note:** World size always reflects the target number of replicas configured for the deployment, not the current number of running replicas. During scaling operations, the world size updates immediately to the new target, even while replicas are still starting or stopping.
...
> **Note:** When a replica crashes, Ray Serve automatically starts a replacement replica and assigns it the **same rank** as the crashed replica. This ensures rank contiguity is maintained without reassigning other replicas.
