---
id: serve.advanced-guides.performance
...
- worker
references:
- /en/latest/serve/api/doc/ray.serve.schema.ControllerHealthMetrics
- /en/latest/serve/api/doc/ray.serve.schema.DurationStats
canonical_path: /en/latest/serve/advanced-guides/performance
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/advanced-guides/performance.md
...
> **Note:** This section offers some tips and tricks to improve your Ray Serve application's performance. Check out the [architecture page](serve-architecture) for helpful context, including an overview of the HTTP proxy actor and deployment replica actors.
...
> **Note:** Serve returns a response with status code `408` when a request times out. Clients can retry when they receive this `408` response.
...
> **Note:** In Ray v2.54.0, the defaults for `RAY_SERVE_RUN_USER_CODE_IN_SEPARATE_THREAD` and `RAY_SERVE_RUN_ROUTER_IN_SEPARATE_LOOP` will change to `0` for improved performance.
