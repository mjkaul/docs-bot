---
id: serve.llm.user-guides.prefill-decode
...
- worker
references: []
canonical_path: /en/latest/serve/llm/user-guides/prefill-decode
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/llm/user-guides/prefill-decode.md
...
> **Note:** The `LMCACHE_CONFIG_FILE` environment variable must point to an existing configuration file that's accessible within the Ray Serve container or worker environment. Ensure these configuration files are properly mounted or available in your deployment environment.
