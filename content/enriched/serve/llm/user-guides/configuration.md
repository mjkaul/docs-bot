---
id: serve.llm.user-guides.configuration
...
- worker
references: []
canonical_path: /en/latest/serve/llm/user-guides/configuration
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/llm/user-guides/configuration.md
license: Apache 2.0, The Ray Authors
---

# Configuration reference

LLMConfig is the central configuration object for a Ray Serve LLM deployment. It describes which model to serve, which engine and hardware to run it on, how to scale it, and how to expose it. You pass one `LLMConfig` per model to a builder such as build_openai_app, or you write the equivalent YAML.

This page documents every field. For the design behind these abstractions, see Core components.
...
|-------|------|---------|-------------|
| `model_loading_config` | dict or ~ray.serve.llm.ModelLoadingConfig | required | Which model to serve and where to load it from. See [Model loading](#model-loading). |
...
| `lora_config` | dict or ~ray.serve.llm.LoraConfig | `None` | LoRA adapter settings. See [LoRA](#lora). |
...
| `server_cls` | str or class | `None` | Custom server class, for example `SGLangServer`. Accepts an import path string or a class. See SGLang integration. |
| `experimental_configs` | dict | `{}` | Experimental knobs. See [Experimental configs](#experimental-configs). |
| `log_engine_metrics` | bool | `True` | Export engine metrics through the Ray Prometheus port. See Observability. |
| `callback_config` | `CallbackConfig` | default | Initialization callbacks. See Deployment initialization. |

## Model loading

`model_loading_config` is validated against ~ray.serve.llm.ModelLoadingConfig:
...
A cloud mirror is validated against ~ray.serve.llm.CloudMirrorConfig and supports `s3://`, `gs://`, `abfss://`, and `azure://` URIs. Mirroring weights from cloud storage avoids per-replica Hugging Face downloads. See Deployment initialization.
...
|----------|-------------|
| `tensor_parallel_size` | Number of GPUs to shard each model replica across. See Cross-node parallelism. |
...
| `kv_transfer_config` | KV connector settings for prefill/decode and cache offloading. See KV cache offloading. |
...
`strategy` is one of `PACK`, `STRICT_PACK`, `SPREAD`, or `STRICT_SPREAD`. See Cross-node parallelism for worked examples.
...
Set `num_replicas="auto"` to let Serve autoscale. Use `request_router_config` to plug in a custom router, for example for prefix-aware routing. For the meaning of each option, see the [Ray Serve deployment configuration](https://docs.ray.io/en/latest/serve/configure-serve-deployment.html) docs.

## LoRA

When `lora_config` is set, the deployment serves LoRA adapters on top of the base model. It is validated against ~ray.serve.llm.LoraConfig:
...
See Multi-LoRA deployment for the full workflow.
...
- Quickstart: deploy and query a model.
- LLMConfig API reference: the generated API docs.
