---
id: serve.llm.user-guides.direct-streaming
...
- replica
references: []
canonical_path: /en/latest/serve/llm/user-guides/direct-streaming
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/llm/user-guides/direct-streaming.md
...
> **Tip:** Ray Serve sets `TCP_NODELAY` by default (`RAY_SERVE_HAPROXY_TCP_NODELAY=1`) so the first streamed chunk isn't held back by Nagle's algorithm. Keep it enabled for streaming workloads.
...
- **Data parallel attention** (`build_dp_openai_app`): the `DPServer` deployment serves the engine app directly. Use this for wide expert parallelism. See data-parallel-attention.
- **Prefill/decode disaggregation** (`build_pd_openai_app`): the decode server serves the engine app directly. See prefill-decode.
...
If you set `request_router_config`, direct streaming uses it as-is. Otherwise it falls back to `RoundRobinRouter`. For the available policies and how to write your own, see routing-policies-guide and custom-request-router-guide.
...
- routing-policies-guide - request routing concepts and available policies
- prefix-aware-routing-guide - cache-locality routing policy
- custom-request-router-guide - implement a custom request router
