---
id: serve.llm.architecture.routing-policies
...
- /en/latest/serve/api/doc/ray.serve.request_router.RequestRouter.on_replica_actor_died
- /en/latest/serve/api/doc/ray.serve.request_router.RequestRouter.on_request_routed
canonical_path: /en/latest/serve/llm/architecture/routing-policies
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/llm/architecture/routing-policies.md
...
> **Note:** Direct streaming uses these same routing policies and the same `request_router_config`, so nothing in this document changes. The only difference is where replica selection runs: at the HTTP ingress instead of inside `OpenAiIngress`. See the direct streaming guide for details and caveats.
...
You can implement custom routing policies by extending Ray Serve's [`RequestRouter`](../../api/doc/ray.serve.request_router.RequestRouter.rst) base class. For detailed examples and step-by-step guides on implementing custom routers, see custom-request-router-guide.
...
Ray Serve provides mixin classes that add common functionality to routers. See the custom-request-router-guide for examples:
...
- prefix-aware-routing-guide - user guide for deploying prefix-aware routing
- custom-request-router-guide - Ray Serve guide for implementing custom routers
