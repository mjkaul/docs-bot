---
id: serve.llm.architecture.core
...
- worker
references: []
canonical_path: /en/latest/serve/llm/architecture/core
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/llm/architecture/core.md
...
- **SGLangServer**: Community-supported integration using SGLang's in-process engine with RadixAttention. See the SGLang integration guide for details.
...
`LLMConfig` is the central configuration object that specifies everything needed to deploy an LLM. The fields below are the most important ones. For a complete field-by-field reference with defaults, see the Configuration reference.
...
- serving-patterns/index - Detailed serving pattern documentation
- routing-policies - Request routing architecture
- ../user-guides/index - Practical deployment guides
