---
id: serve.tutorials.serve-ml-models
...
- deployment
references: []
canonical_path: /en/latest/serve/tutorials/serve-ml-models
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/tutorials/serve-ml-models.md
...
> **Note:** `ImageModel.bind()` returns a `DeploymentNode` object, a wrapping of the `ImageModel` deployment object, that you can then use to connect with other `DeploymentNodes` to form a more complex [deployment graph](serve-model-composition).
...
> **Note:** `BoostingModel.bind(MODEL_PATH, LABEL_PATH)` binds the arguments `MODEL_PATH` and `LABEL_PATH` to the deployment and returns a `DeploymentNode` object, a wrapping of the `BoostingModel` deployment object, that you can then use to connect with other `DeploymentNodes` to form a more complex [deployment graph](serve-model-composition).
