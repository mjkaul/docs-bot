---
id: serve.tutorials.gradio-integration
...
- replica
references: []
canonical_path: /en/latest/serve/tutorials/gradio-integration
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/tutorials/gradio-integration.md
...
> **Note:** Ray Serve doesn't support routing requests to multiple replicas of `GradioServer`, so you should only have a single replica.
...
> **Note:** Ray can’t pickle Gradio. Instead, pass a builder function that constructs the Gradio interface.
...
> **Note:** This step binds the two text generation models, which you wrapped in Serve deployments, to `MyGradioServer._d1` and `MyGradioServer._d2`, forming a [model composition](serve-model-composition). In the example, the Gradio Interface `io` calls `MyGradioServer.fanout()`, which sends requests to the two text generation models that you deployed on Ray Serve.
