...
- serve-handle
references: []
canonical_path: /en/latest/serve/architecture
source: https://github.com/ray-project/ray/blob/master/doc/source/serve/architecture.md
...
> **Note:** When the controller dies, requests can still be sent via HTTP, gRPC and `DeploymentHandle`, but autoscaling is paused. When the controller recovers, the autoscaling resumes, but all previous metrics collected are lost.
