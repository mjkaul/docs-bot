...
- task
references: []
canonical_path: /en/latest/data/loading-data
source: https://github.com/ray-project/ray/blob/master/doc/source/data/loading-data.rst
...
> **Tip:** When reading parquet files, you can take advantage of column pruning to
...
on the projection pushdown feature.
        ...
        To read raw images, call ~ray.data.read_images. Ray Data represents
...
To read files from local disk, call a function like ~ray.data.read_parquet and
...
> **Tip:** If your files are accessible on every node, exclude `local://` to parallelize the
...
Then, call a method like ~ray.data.read_parquet and specify URIs with the
...
You can download data in bulk by leveraging the ~ray.data.Dataset.with_column method together with the ~ray.data.expressions.download expression. This approach lets the system handle the parallel downloading of files referenced by URLs in your dataset, without needing to manage async code within your own transformations.
...
Ray Data interoperates with libraries like pandas, NumPy, and Arrow.
        ...
        To create a ~ray.data.dataset.Dataset from Python objects, call
        ~ray.data.from_items and pass in a list of `Dict`. Ray Data treats
        each `Dict` as a row.
        ...
        You can also create a ~ray.data.dataset.Dataset from a list of regular
        ...
        To create a ~ray.data.dataset.Dataset from a NumPy array, call
        ~ray.data.from_numpy. Ray Data treats the outer axis as the row
        dimension.
        ...
        To create a ~ray.data.dataset.Dataset from a pandas DataFrame, call
        ...
        To create a ~ray.data.dataset.Dataset from an Arrow table, call
        ~ray.data.from_arrow.
...
Ray Data interoperates with distributed data processing frameworks like [Daft](https://www.getdaft.io),
Dask, Spark, Modin, and
Mars.

> **Note:** The Ray Community provides these operations but may not actively maintain them. If you run into issues,
create a GitHub issue [here](https://github.com/ray-project/ray/issues).

    
        To create a ~ray.data.dataset.Dataset from a [Daft DataFrame](https://docs.getdaft.io/en/stable/api/dataframe/), call
        ~ray.data.from_daft. This function executes the Daft dataframe and constructs a `Dataset` backed by the resultant arrow data produced
        by your Daft query.

        
> **Warning:** ~ray.data.from_daft doesn't work with PyArrow 14 and later. For more
information, see [this issue](https://github.com/ray-project/ray/issues/54837).
...
:skipif: True
        ...
        To create a ~ray.data.dataset.Dataset from a
        [Dask DataFrame](https://docs.dask.org/en/stable/dataframe.html), call
        ~ray.data.from_dask. This function constructs a
...
:skipif: True
        ...
        To create a ~ray.data.dataset.Dataset from a [Spark DataFrame](https://spark.apache.org/docs/latest/api/python/reference/pyspark.sql/dataframe.html),
        call ~ray.data.from_spark. This function creates a `Dataset` backed by
...
:skipif: True
        ...
        To create a ~ray.data.dataset.Dataset from an [Iceberg Table](https://iceberg.apache.org),
        call ~ray.data.read_iceberg. This function creates a `Dataset` backed by
...
:skipif: True
        ...
        To create a ~ray.data.dataset.Dataset from a Modin DataFrame, call
        ~ray.data.from_modin. This function constructs a `Dataset` backed by
        ...
        To create a ~ray.data.dataset.Dataset from a Mars DataFrame, call
        ~ray.data.from_mars. This function constructs a `Dataset`
...
To read datasets from the Hugging Face Hub, use ~ray.data.read_parquet (or other
...
> **Tip:** If you encounter serialization errors when reading from Hugging Face filesystems, try upgrading `huggingface_hub` to version 1.1.6 or later. For more details, see this issue: https://github.com/ray-project/ray/issues/59029
...
Ray Data interoperates with PyTorch and TensorFlow datasets.
        ...
        with ~ray.data.read_parquet, ~ray.data.read_csv, or ~ray.data.read_json.
...
:skipif: True
        ...
        To convert a PyTorch dataset to a Ray Dataset, call ~ray.data.from_torch.
...
> **Warning:** ~ray.data.from_tf doesn't support parallel reads. Only use this
...
Call ~ray.data.read_sql to read data from a database that provides a
...
:skipif: True
        ...
        To read data from BigQuery, call ~ray.data.read_bigquery and specify the project id, dataset, and query (if applicable).
...
To read data from MongoDB, call ~ray.data.read_mongo and specify
...
To read data from Kafka topics, call ~ray.data.read_kafka and specify
...
Synthetic datasets can be useful for testing and benchmarking.
        ...
        To create a synthetic ~ray.data.Dataset from a range of integers, call
        ~ray.data.range. Ray Data stores the integer range in a single column called
        "id".
        ...
        To create a synthetic ~ray.data.Dataset containing arrays, call
        ~ray.data.range_tensor. Ray Data packs an integer range into ndarrays of
...
~ray.data.Datasource. Then, construct an instance of your custom
datasource and pass it to ~ray.data.read_datasource. To write results, you might
also need to subclass ray.data.Datasink. Then, create an instance of your custom
datasink and pass it to ~ray.data.Dataset.write_datasink. For more details, see
...
for optimizing read performance, see [Optimizing reads](performance-tips.html#optimizing-reads).
