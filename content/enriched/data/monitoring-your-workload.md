---
id: data.monitoring-your-workload
title: Monitoring Your Workload
topic_type: task
description: ''
subjects:
- data
...
- task
references: []
canonical_path: /en/latest/data/monitoring-your-workload
source: https://github.com/ray-project/ray/blob/master/doc/source/data/monitoring-your-workload.rst
license: Apache 2.0, The Ray Authors
---

Monitoring Your Workload

This section helps you debug and monitor the execution of your ~ray.data.Dataset by viewing the:
...
When you execute a ~ray.data.Dataset, Ray Data displays a set of progress bars in the console. These progress bars show various execution and progress-related metrics, including the number of rows completed/remaining, resource usage, and task/actor status. See the annotated image for a breakdown of how to interpret the progress bar outputs: 
...
> **Tip:** There is a new experimental console UI to show progress bars. Set `DataContext.get_current().enable_rich_progress_bars = True` or set the `RAY_DATA_ENABLE_RICH_PROGRESS_BARS=1` environment variable to enable.
...
> **Note:** Most metrics are only available for physical operators that use the map operation. For example, physical operators created by ~ray.data.Dataset.map_batches, ~ray.data.Dataset.map, and ~ray.data.Dataset.flat_map.
...
This log file can be found locally at `/tmp/ray/{SESSION_NAME}/logs/ray-data/ray-data.log`. It can also be found on the Ray Dashboard under the head node's logs in the Logs view.

Ray Data stats

To see detailed stats on the execution of a dataset you can use the ~ray.data.Dataset.stats method.
...
* **UDF time**: The UDF time is time spent in functions defined by the user. This time includes functions you pass into Ray
  Data methods, including ~ray.data.Dataset.map, ~ray.data.Dataset.map_batches, ~ray.data.Dataset.filter,
...
might see iteration stats, for example, if you call ~ray.data.Dataset.take_all. Some of the stats that Ray Data includes
...
As a concrete example, below is a stats output from Image Classification Batch Inference with PyTorch ResNet18:
