...
- worker
references: []
canonical_path: /en/latest/data/transforming-data
source: https://github.com/ray-project/ray/blob/master/doc/source/data/transforming-data.rst
...
> **Note:** Transformations are lazy by default. They aren't executed until you trigger consumption of the data by iterating over the Dataset, saving the Dataset, or inspecting properties of the Dataset.

This guide shows you how to scale transformations (or user-defined functions (UDFs)) on your Ray Data dataset.

Transforming rows

> **Tip:** If your transformation is vectorized, call ~ray.data.Dataset.map_batches for
...
~ray.data.Dataset.map. This transformation is automatically parallelized across your Ray cluster.
...
The user defined function passed to ~ray.data.Dataset.map should be of type
...
~ray.data.Dataset.flat_map. This transformation is automatically parallelized across your Ray cluster.
...
The user defined function passed to ~ray.data.Dataset.flat_map should be of type
...
specify `batch_format` in ~ray.data.Dataset.map_batches. You can return either
...
2. Call ~ray.data.Dataset.map_batches, ~ray.data.Dataset.map, or
   ~ray.data.Dataset.flat_map. Pass a `ray.data.ActorPoolStrategy(...)` object to
...
You can read more about resources in Ray here: resource-requirements.
...
you can use ~ray.data.Dataset.sort method, or set ray.data.ExecutionOptions.preserve_order to `True`.
...
To transform groups, call ~ray.data.Dataset.groupby to group rows based on provided `key` column values.
Then, call ~ray.data.grouped_data.GroupedData.map_groups to execute a transformation on each group.
...
You can do this by using placement groups and `ray_remote_args_fn`, which can dynamically create placement groups for each model replica.
...
Use ~ray.data.expressions.col to reference columns and ~ray.data.expressions.lit to create literal values.
You can combine these with operators to create complex expressions for filtering,
transformations, and computations.

Expressions have to be used with ~ray.data.Dataset.with_column. The core advantage of expressions
...
To use a custom function with an expression, you can use ~ray.data.expressions.udf.
