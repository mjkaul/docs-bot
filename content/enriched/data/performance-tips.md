---
id: data.performance-tips
title: 'Advanced: Performance Tips and Tuning'
topic_type: concept
description: ''
subjects:
- data
...
- task
references: []
canonical_path: /en/latest/data/performance-tips
source: https://github.com/ray-project/ray/blob/master/doc/source/data/performance-tips.rst
...
~ray.data.Dataset.map_batches rather than ~ray.data.Dataset.map. It's
faster.
...
You can speed up ~ray.data.Dataset.sort and operations that sort internally,
such as ~ray.data.grouped_data.GroupedData.map_groups, by enabling Polars:
...
This flag doesn't affect other operations such as ~ray.data.Dataset.map_batches.
...
- Usually, if the read is followed by a ~ray.data.Dataset.map or ~ray.data.Dataset.map_batches, the map is fused with the read; therefore `override_num_blocks` also determines the number of map tasks.

Ray Data decides the default value for number of output blocks based on the following heuristics, applied in order:

1. Start with the default value of 200. You can overwrite this by setting DataContext.read_op_min_num_blocks.
2. Min block size (default=1 MiB). If number of blocks would make blocks smaller than this threshold, reduce number of blocks to avoid the overhead of tiny blocks. You can override by setting DataContext.target_min_block_size (bytes).
3. Max block size (default=128 MiB). If number of blocks would make blocks larger than this threshold, increase number of blocks to avoid out-of-memory errors during processing. You can override by setting DataContext.target_max_block_size (bytes).
...
Thus, the total blocks in the final ~ray.data.Dataset may differ from the specified `override_num_blocks`.
...
For example, the following code executes ~ray.data.read_csv with only one task, but its output is split into 4 blocks before executing the ~ray.data.Dataset.map:
...
calling ~ray.data.Dataset.select_columns, since column selection is pushed down to the file scan.
...
1. An all-to-all shuffle operation is used. Or,
2. There is a call to ds.materialize().
...
1. If you need control over the exact number of output blocks, use ds.repartition(num_partitions). Note that this is an all-to-all operation and it materializes all blocks into memory before performing the repartition.
2. If you don't need control over the exact number of output blocks and just want to produce larger blocks, use ds.map_batches(lambda batch: batch, batch_size=batch_size) and set `batch_size` to the desired number of rows per block. This is executed in a streaming fashion and avoids materialization.
