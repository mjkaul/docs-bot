---
id: data.scaling-collation-functions
title: 'Advanced: Scaling out expensive collate functions'
topic_type: concept
description: ''
subjects:
- data
...
- worker
references: []
canonical_path: /en/latest/data/scaling-collation-functions
source: https://github.com/ray-project/ray/blob/master/doc/source/data/scaling-collation-functions.rst
license: Apache 2.0, The Ray Authors
---

Advanced: Scaling out expensive collate functions

By default, the collate function executes on the training worker when you call ray.data.DataIterator.iter_torch_batches. This approach has two main drawbacks:
...
* Create a custom collate function that runs in Ray Data and use ray.data.Dataset.map_batches to scale it out.
* Use ray.data.Dataset.repartition to ensure the batch size alignment.
...
However, if you move the collate function to Ray Data using ray.data.Dataset.map_batches, it doesn't guarantee the batch size for each function call by default.
...
To solve these problems, you can use ray.data.Dataset.repartition with `target_num_rows_per_block` to ensure the batch size alignment.
...
This guide uses a mock text dataset to demonstrate the optimization. You can find the implementation of the mock dataset in random-text-generator.
...
The preceding optimized implementation returns `Dict[str, np.ndarray]`, which Ray Data natively supports. However, if your collate function needs to return PyTorch tensors or other custom data types that ray.data.Dataset.map_batches doesn't directly support, you need to serialize them.
...
You can use train-collate-utils as a reference implementation and adapt it to your needs.
