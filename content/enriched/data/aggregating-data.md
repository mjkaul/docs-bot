...
- dataset
references: []
canonical_path: /en/latest/data/aggregating-data
source: https://github.com/ray-project/ray/blob/master/doc/source/data/aggregating-data.rst
license: Apache 2.0, The Ray Authors
---

Aggregating Data

Ray Data provides a flexible and performant API for performing aggregations on ~ray.data.dataset.Dataset. 

Basic Aggregations

Ray Data provides several built-in aggregation functions like ~ray.data.Dataset.max,
~ray.data.Dataset.min, ~ray.data.Dataset.sum.
...
The full list of built-in aggregation functions is available in the Dataset API reference.

Each of the preceding methods also has a corresponding AggregateFnV2 object. These objects can be used in ~ray.data.Dataset.aggregate() or Dataset.groupby().aggregate().
...
You can create custom aggregations by implementing the ~ray.data.aggregate.AggregateFnV2 interface. The AggregateFnV2 interface has three key methods to implement:
...
> **Note:** Internally, aggregations support both the hash-shuffle backend and the range based backend.

    Hash-shuffling can provide better performance for aggregations in certain cases. For more information see [comparison between hash based shuffling and Range Based shuffling approach](https://www.anyscale.com/blog/ray-data-joins-hash-shuffle#performance-benchmarks/) .
