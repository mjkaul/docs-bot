...
- worker
references: []
canonical_path: /en/latest/data/mixing-data
source: https://github.com/ray-project/ray/blob/master/doc/source/data/mixing-data.rst
...
You can compose ~ray.data.Dataset.mix with other Ray Data operations to implement different mixing strategies, depending on how granular you want the mixing ratio to be. The sections below cover **per-block mixing** (~ray.data.Dataset.mix on its own) and **random mixing** (~ray.data.Dataset.mix followed by a shuffle).

Per-block mixing

By default, each output block comes from exactly one input dataset. ~ray.data.Dataset.mix keeps a running row count per source and, on every step, pulls the next block from whichever dataset is furthest behind its target ratio. Over time, the cumulative row counts converge to the requested weights.
...
> **Note:** You may want to repartition to some multiple of batch size (for example, `N * LOCAL_BATCH_SIZE`) if your rows are small in terms of bytes. This prevents splitting blocks into extremely small pieces that increase overhead.
...
Adding a streaming shuffle after ~ray.data.Dataset.mix switches you to **random mixing**: the shuffle redistributes rows across block boundaries so each batch directly contains rows from multiple datasets in roughly the requested proportion, regardless of how many workers you're training on. ~ray.data.Dataset.mix still governs the ratio; the shuffle just spreads it within each batch.

Two streaming-friendly shuffle options in Ray Data:

- Local buffer shuffle (~ray.data.DataIterator.iter_batches with `local_shuffle_buffer_size`)
- map_batches shuffle
...
See ~ray.data.MixStoppingCondition for more details.

Limitations

- **Avoid** ~ray.data.Dataset.map / ~ray.data.Dataset.filter **after** ~ray.data.Dataset.mix. Downstream transformations can combine or split blocks before they reach the trainer, which breaks the row-ratio guarantees ~ray.data.Dataset.mix provides. Apply per-dataset transforms upstream of ~ray.data.Dataset.mix.
- **Schemas must match.** ~ray.data.Dataset.mix does not unify schemas for you. Apply ~ray.data.Dataset.map or ~ray.data.Dataset.select_columns upstream to make all inputs structurally identical.
...
- Using Ray Data with Ray Train for distributed training and data ingest
- Ray Data shuffling solutions
