---
id: tune.tutorials.tune-distributed
title: Running Distributed Experiments with Ray Tune
topic_type: task
description: ''
subjects:
- tune
...
- worker
references: []
canonical_path: /en/latest/tune/tutorials/tune-distributed
source: https://github.com/ray-project/ray/blob/master/doc/source/tune/tutorials/tune-distributed.rst
...
2. Run the script on the head node, or use ray submit, or use Ray Job Submission.
...
> **Tip:** 1. In the examples, the Ray address commonly used is `localhost:6379`.
...
Tune automatically persists the progress of your entire experiment (a `Tuner.fit()` session), so if an experiment crashes or is otherwise cancelled, it can be resumed through ~ray.tune.Tuner.restore.
