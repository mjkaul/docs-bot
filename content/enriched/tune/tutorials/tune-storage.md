---
id: tune.tutorials.tune-storage
title: How to Configure Persistent Storage in Ray Tune
topic_type: task
description: ''
subjects:
- tune
...
- trial
references: []
canonical_path: /en/latest/tune/tutorials/tune-storage
source: https://github.com/ray-project/ray/blob/master/doc/source/tune/tutorials/tune-storage.rst
...
> **Note:** A network filesystem or cloud storage can be configured for single-node
...
after termination.
    ...
    See ~ray.tune.SyncConfig for the full set of configuration options as well as more details.
...
> **Note:** The head node will not have access to all experiment results locally. If you want to process
...
unless you customize this with `storage_path` and `name` in ~ray.tune.RunConfig.
...
> **Warning:** When running on multiple nodes, using the local filesystem of the head node as the persistent storage location is *deprecated*.
...
See tune-fault-tolerance-ref for more information on resuming experiments.
...
See tune-persisted-experiment-data for an overview of other experiment data that Tune needs to persist.
...
~ray.tune.Tuner.restore for more details.
