---
id: tune.tutorials.tune-output
title: Logging and Outputs in Tune
topic_type: task
description: ''
subjects:
- tune
...
- worker
references: []
canonical_path: /en/latest/tune/tutorials/tune-output
source: https://github.com/ray-project/ray/blob/master/doc/source/tune/tutorials/tune-output.rst
...
You can learn more about logging and customizations here: loggers-docstring.
...
To learn more about Trials, see its detailed API documentation: trial-docstring.
...
the head process (see ray worker logs for more information).
...
> **Note:** This can cause problems when the trainable is moved across different nodes throughout its lifetime.
...
> **Note:** If SyncConfig(sync_artifacts=True), trial artifacts
are uploaded periodically from each trial (or from each remote training worker for Ray Train)
to the RunConfig(storage_path).

    See the ~ray.tune.SyncConfig API reference for artifact syncing configuration options.
...
You can create a custom logger by inheriting the LoggerCallback interface (logger-interface):
...
An example of creating a custom logger can be found in /tune/examples/includes/logging_example.
