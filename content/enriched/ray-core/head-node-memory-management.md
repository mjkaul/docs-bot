---
id: ray-core.head-node-memory-management
title: Head Node Memory Management
topic_type: concept
description: ''
subjects:
- core
...
- worker
references: []
canonical_path: /en/latest/ray-core/head-node-memory-management
source: https://github.com/ray-project/ray/blob/master/doc/source/ray-core/head-node-memory-management.rst
...
- The Ray Dashboard provides a web interface for cluster monitoring and debugging. For more details, see observability-getting-started.
...
See vms-large-cluster-configure-head-node for head-node best practices.
...
> **Warning:** Disabling the dashboard prevents KubeRay's `RayJob` and `RayService` features from working properly.
...
> **Note:** You *can* disable the dashboard, but doing so severely limits observability and isn't **recommended for production**. If you choose to disable it, see the `Disable the Dashboard` section in the preceding text.

Troubleshooting

If your head node experiences OOM issues:  
  
1. Check current memory usage: `ray memory`. See debug-with-ray-memory
2. Consider increasing head node memory allocation

For more information on OOM prevention, see ray-oom-prevention.
