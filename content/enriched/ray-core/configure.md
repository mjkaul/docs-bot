...
- worker
references: []
canonical_path: /en/latest/ray-core/configure
source: https://github.com/ray-project/ray/blob/master/doc/source/ray-core/configure.rst
...
and from the command line. Take a look at the `ray.init` [documentation](package-ref.html#ray.init) for a complete overview of the configurations.
...
`%Y-%m-%d_%H-%M-%S_%f` (See [Python time format](strftime.org) for details);
