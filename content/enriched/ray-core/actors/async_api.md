---
id: ray-core.actors.async_api
title: AsyncIO / Concurrency for Actors
topic_type: concept
description: ''
subjects:
- core
...
- task
references: []
canonical_path: /en/latest/ray-core/actors/async_api
source: https://github.com/ray-project/ray/blob/master/doc/source/ray-core/actors/async_api.rst
...
**Neither the** threaded-actors nor async-actors **model will allow you to bypass the GIL.**
...
In async actors, only one task can be running at any point in time (though tasks can be multiplexed). There will be only one thread in AsyncActor! See threaded-actors if you want a threadpool.
...
> **Warning:** When there is at least one `async def` method in actor definition, Ray
