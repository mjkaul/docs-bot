---
id: ray-core.scheduling.placement-group
...
- trial
references: []
canonical_path: /en/latest/ray-core/scheduling/placement-group
source: https://github.com/ray-project/ray/blob/master/doc/source/ray-core/scheduling/placement-group.rst
...
A **placement group** reserves the resources from the cluster. Tasks or actors must use the PlacementGroupSchedulingStrategy to use the reserved resources.
...
> **Note:** When a placement group with GPUs is reserved successfully, the bundles are not necessarily ordered by GPU physical rank.
...
> **Note:** By default, Ray actors require 1 logical CPU at schedule time, but after being scheduled, they do not acquire any CPU resources.
...
> **Note:** When you remove the placement group, actors or tasks that still use the reserved resources are
forcefully killed.
...
> **Note:** Ray dashboard is only available when you install Ray is with `pip install "ray[default]"`.
...
> **Note:** State API is only available when you install Ray is with `pip install "ray[default]"`
...
Ray reschedules Actors and tasks that use the bundle (reserved resources) based on their fault tolerant policy once Ray recovers the bundle.

[Alpha] Label locality scheduling

> **Warning:** Label locality scheduling is an **alpha** feature. It's actively being iterated on and
...
You could work around this with static label selectors (such as
...
> **Note:** Ray doesn't automatically set the `ray.io/gpu-domain` label on nodes.
...
- **Partial failure** (some nodes in the domain die): Ray reschedules the lost bundles
  ...
  ray.util.remove_placement_group and
