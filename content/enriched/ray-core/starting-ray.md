...
- worker
references: []
canonical_path: /en/latest/ray-core/starting-ray
source: https://github.com/ray-project/ray/blob/master/doc/source/ray-core/starting-ray.rst
...
* Implicitly via `ray.init()` (start-ray-init)
* Explicitly via CLI (start-ray-cli)
* Explicitly via the cluster launcher (start-ray-up)
...
> **Note:** In recent versions of Ray (>=1.5), `ray.init()` will automatically be called on the first use of a Ray remote API.
...
> **Note:** The behavior of `ray.shutdown()` depends on how the cluster was initialized:
...
See the [Configuration](configure.html) documentation for the various ways to configure Ray.
...
You can connect other nodes to the head node, creating a Ray cluster by also calling `ray start` on those nodes. See on-prem for more details. Calling `ray.init()` on any of the cluster machines will connect to the same Ray cluster.
...
Check out our [Deployment section](../cluster/getting-started.html) for more information on deploying Ray in different settings, including [Kubernetes](../cluster/kubernetes/index.html), [YARN](../cluster/vms/user-guides/community/yarn.html), and [SLURM](../cluster/vms/user-guides/community/slurm.html).
