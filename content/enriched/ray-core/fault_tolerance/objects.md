---
id: ray-core.fault_tolerance.objects
...
- worker
references: []
canonical_path: /en/latest/ray-core/fault_tolerance/objects
source: https://github.com/ray-project/ray/blob/master/doc/source/ray-core/fault_tolerance/objects.rst
...
* Tasks will only be re-executed up to their maximum number of retries. By
  ...
  for remote functions and the
  `max_task_retries` parameter for actors.
...
- `ObjectReconstructionFailedError`: This error is thrown if an object, or
  ...
  one of the limitations described above.
- `ReferenceCountingAssertionError`: The object has already been deleted,
  ...
  However, there is a [known edge case](https://github.com/ray-project/ray/issues/18456) that can produce this error.
