---
id: ray-core.internals.autoscaler-v2
title: Autoscaler v2
topic_type: concept
description: ''
subjects:
...
- worker
references: []
canonical_path: /en/latest/ray-core/internals/autoscaler-v2
source: https://github.com/ray-project/ray/blob/master/doc/source/ray-core/internals/autoscaler-v2.rst
...
> **Note:** Although the autoscaler fulfills pending resource demands and releases idle nodes, it doesn't perform the actual scheduling of Ray tasks, actors, or placement groups. Scheduling is handled internally by Ray.
...
> **Warning:** In the case of the cluster launcher, if the autoscaler process crashes, then there is no autoscaling.
...
> **Warning:** If any error occurs, such as an error from the cloud instance provider or a timeout in the sync phase, the current reconciliation is aborted and the loop jumps to step 7 to wait for the next reconciliation.

> **Note:** All scaling decisions from steps 1–5 are accumulated purely in memory.
...
> **Note:** Utilization rate used by feasible requests is calculated as the difference between the total and available resources divided by the total resources.
...
> **Note:** The drain request sent by `RAY_STOP_REQUESTED` can be rejected if the node is no longer idle when the drain request arrives the node. Then the instance will be transitioned back to `RAY_RUNNING` instead.
...
> **Note:** These transitions trigger side effects, but side effects don't trigger new transitions directly.
...
> **Note:** Cloud instance provider implementations in autoscaler v2 must implement:
