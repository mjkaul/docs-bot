---
id: train.getting-started-jax
title: Get Started with Distributed Training using JAX
topic_type: task
description: ''
subjects:
- train
...
- worker
references: []
canonical_path: /en/latest/train/getting-started-jax
source: https://github.com/ray-project/ray/blob/master/doc/source/train/getting-started-jax.rst
license: Apache 2.0, The Ray Authors
---

Get Started with Distributed Training using JAX

This guide provides an overview of the ~ray.train.v2.jax.JaxTrainer in Ray Train.
...
JAX and ~ray.train.v2.jax.JaxTrainer support different accelerators such as GPUs and TPUs. For more details, see [JAX Supported platforms](https://docs.jax.dev/en/latest/installation.html#supported-platforms).
...
To learn more about configuring TPUs with KubeRay, see kuberay-tpu.

JaxTrainer API

The ~ray.train.v2.jax.JaxTrainer is the core component for orchestrating distributed JAX training in Ray Train.
...
For TPU training, use ~ray.train.ScalingConfig to define your TPU slices configuration. Key fields include:

* use_tpu: It's a new field added in Ray 2.49.0 to the V2 `ScalingConfig`. This boolean flag tells Ray Train to initialize the JAX backend for TPU execution.
* topology: It's a new field added in Ray 2.49.0 to the V2 `ScalingConfig`. Topology is a string defining the physical arrangement of the TPU chips (for example, "4x4"). It's required for multi-host training and ensures Ray places workers correctly across the slice. For a list of supported TPU topologies by generation,
  see the [GKE documentation](https://cloud.google.com/kubernetes-engine/docs/concepts/plan-tpus#topology).
* num_workers: Set this to the total number of TPU VMs across all slices. For example, one v4-32 slice with a 2x2x4 topology uses 4 VMs, so set `num_workers` to 4. If you use two v4-32 slices, set `num_workers` to 8.
* resources_per_worker: A dictionary specifying the resources each worker needs. For TPUs, you typically request the number of chips per VM (for example, `{"TPU": 4}`).
* accelerator_type: For TPUs, `accelerator_type` specifies the TPU generation you're using (for example, "TPU-V6E"), ensuring your workload is scheduled on the desired TPU slice.
...
For GPU training, use ~ray.train.ScalingConfig to define your GPU configuration.
...
* use_gpu: Whether each worker should use a GPU.
* resources_per_worker: A dictionary specifying the resources each worker needs.
...
2. ~ray.train.ScalingConfig defines the number of distributed training workers and whether to use TPUs or GPUs.
3. ~ray.train.v2.jax.JaxTrainer launches the distributed training job.
...
Tying it all together, you can now launch a distributed training job with a ~ray.train.v2.jax.JaxTrainer.
