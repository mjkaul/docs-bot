---
id: train.getting-started-transformers
title: Get Started with Distributed Training using Hugging Face Transformers
topic_type: task
description: ''
subjects:
- train
...
- worker
references: []
canonical_path: /en/latest/train/getting-started-transformers
source: https://github.com/ray-project/ray/blob/master/doc/source/train/getting-started-transformers.rst
...
1. Configure a training function that properly reports metrics and saves checkpoints.
2. Configure scaling and resource requirements for CPUs or GPUs for your distributed training job.
3. Launch a distributed training job with ~ray.train.torch.TorchTrainer.
...
2. ~ray.train.ScalingConfig: Defines the number of distributed training workers and GPU usage.
3. ~ray.train.torch.TorchTrainer: Launches and manages the distributed training job.
...
> **Note:** When using Hugging Face Datasets or Evaluate, always call `datasets.load_dataset` and `evaluate.load`
...
~ray.train.huggingface.transformers.prepare_trainer to validate
...
Ray 2.7 introduced the unified ~ray.train.torch.TorchTrainer API,
