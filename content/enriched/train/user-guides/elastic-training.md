---
id: train.user-guides.elastic-training
...
- worker
references: []
canonical_path: /en/latest/train/user-guides/elastic-training
source: https://github.com/ray-project/ray/blob/master/doc/source/train/user-guides/elastic-training.rst
...
To enable elastic training, use ~ray.train.ScalingConfig.num_workers to specify `(min_workers, max_workers)` as a tuple instead of a fixed worker group size. You should also set ~ray.train.FailureConfig.max_failures so that training can recover from worker failures instead of exiting immediately.
...
If the cluster gets more nodes eventually, Ray Train can resize the worker group and restart with the new workers added, so training can use the extra capacity. By default, the controller considers resizing every 60 seconds while the worker group is healthy. To change how often resize decisions are made, set ~ray.train.ScalingConfig.elastic_resize_monitor_interval_s in your scaling config:
...
> **Note:** If the Kubernetes cluster itself doesn't have enough physical nodes, you also need to configure a Kubernetes-level autoscaler (such as the Cluster Autoscaler or Karpenter) so that new Kubernetes nodes are provisioned for the Ray worker pods. See kuberay-autoscaling-config for more details.
...
:emphasize-lines: 5,6
        ...
        See vms-autoscaling for more details.
