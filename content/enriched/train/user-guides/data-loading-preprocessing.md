---
id: train.user-guides.data-loading-preprocessing
...
- worker
references: []
canonical_path: /en/latest/train/user-guides/data-loading-preprocessing
source: https://github.com/ray-project/ray/blob/master/doc/source/train/user-guides/data-loading-preprocessing.rst
...
> **Note:** In addition to Ray Data, you can continue to use framework-native data utilities with Ray Train, such as PyTorch Dataset, Hugging Face Dataset, and Lightning DataModule.
...
Your preprocessed datasets can be passed into a Ray Train Trainer (e.g. ~ray.train.torch.TorchTrainer) through the `datasets` argument.
...
Ray Data splits all datasets across the training workers by default. ~ray.train.get_dataset_shard returns `1/n` of the dataset, where `n` is the number of training workers.

Ray Data does data splitting in a streaming fashion on the fly.

> **Note:** Be aware that because Ray Data splits the evaluation dataset, you have to aggregate the evaluation results across workers.
You might consider using [TorchMetrics](https://torchmetrics.readthedocs.io/en/latest/) (example) or
...
- To create a generic Iterable of batches, you can call ~ray.data.DataIterator.iter_batches.
- To create a replacement for a PyTorch DataLoader, you can call ~ray.data.DataIterator.iter_torch_batches.
...
For more details, see the following sections for each framework:
        ...
        **Option 1 (with Ray Data):**
        ...
        3. Inside your `train_loop_per_worker`, you can access the dataset via ray.train.get_dataset_shard.
        4. Create a dataset iterable via ray.data.DataIterator.iter_torch_batches.
        ...
        **Option 2 (without Ray Data):**
        ...
        2. Use the ray.train.torch.prepare_data_loader utility to set up the DataLoader for distributed training.
        ...
        **Option 1 (with Ray Data):**
        ...
        1. Convert your Hugging Face Dataset to a Ray Dataset. For instructions, see Ray Data for Hugging Face.
        2. Pass the Ray Dataset into the TorchTrainer via the `datasets` argument.
        3. Inside your `train_loop_per_worker`, access the sharded dataset via ray.train.get_dataset_shard.
        4. Create a iterable dataset via ray.data.DataIterator.iter_torch_batches.
        5. Pass the iterable dataset while initializing `transformers.Trainer`.
        6. Wrap your transformers trainer with the ray.train.huggingface.transformers.prepare_trainer utility.
...
> **Tip:** When using Torch or Hugging Face Datasets directly without Ray Data, make sure to instantiate your Dataset *inside* the `train_loop_per_worker`.
...
> **Note:** When using PyTorch DataLoader with more than 1 worker, you should set the
...
> **Note:** This section is for tabular/structured data. The recommended way for preprocessing unstructured data is to use
...
object store, which can spill to disk, which in turn decreases
...
* **Use fewer CPUs for data production**: If you are using ~ray.data.Dataset.map_batches,
...
See data_performance_tips for more info on how to tune Ray Data.
