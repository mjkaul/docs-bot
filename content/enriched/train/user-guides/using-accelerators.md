---
id: train.user-guides.using-accelerators
title: Configuring Scale and Accelerators
topic_type: task
description: ''
subjects:
- train
...
- worker
references: []
canonical_path: /en/latest/train/user-guides/using-accelerators
source: https://github.com/ray-project/ray/blob/master/doc/source/train/user-guides/using-accelerators.rst
...
The main interface for this is the ~ray.train.ScalingConfig,
...
which is a Ray Actor that runs your training function.
...
Using accelerators
        ...
        To use GPUs, pass `use_gpu=True` to the ~ray.train.ScalingConfig.
        ...
        To use TPUs, pass `use_tpu=True` to the ~ray.train.ScalingConfig.
...
Using accelerators in the training function
        ...
        depends on the trainer you use (such as ~ray.train.v2.jax.JaxTrainer).
...
> **Tip:** Ensure that your cluster has instances with the specified accelerator type
...
If you explicitly want to override this setting, you can configure a ~ray.train.torch.TorchConfig
and pass it into the ~ray.train.torch.TorchTrainer.
...
> **Note:** If you specify GPUs in `resources_per_worker`, you also need to set
`use_gpu=True`.
...
> **Important:** This API is deprecated. See [this migration guide](https://github.com/ray-project/ray/issues/49454) for more details.
