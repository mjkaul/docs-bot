---
id: train.user-guides.local_mode
title: Local Mode
topic_type: task
description: ''
subjects:
...
- worker
references: []
canonical_path: /en/latest/train/user-guides/local_mode
source: https://github.com/ray-project/ray/blob/master/doc/source/train/user-guides/local_mode.rst
license: Apache 2.0, The Ray Authors
---

Local Mode

> **Important:** This user guide shows how to use local mode with Ray Train V2 only.
...
You can enable local mode by setting `num_workers=0` in your ~ray.train.ScalingConfig:
...
> **Note:** In local mode, Ray Train doesn't launch worker actors, but your training code can still 
...
> **Note:** Local mode works with all Ray Train framework integrations, including PyTorch Lightning,
...
> **Warning:** Ray Data isn't supported when using `torchrun` for multi-process training in local mode. 
...
training accordingly. You can access distributed training information through ray.train.get_context():
...
* **Worker-level fault tolerance**: Ray Train's automatic fault tolerance features, such as worker restart on failure, aren't available. If you configured ~ray.train.FailureConfig, the settings don't apply in local mode.
* **Callbacks**: User-defined callbacks specified in ~ray.train.RunConfig aren't invoked in local mode.
