---
id: train.getting-started-pytorch
title: Get Started with Distributed Training using PyTorch
topic_type: task
description: ''
subjects:
- train
...
- worker
references: []
canonical_path: /en/latest/train/getting-started-pytorch
source: https://github.com/ray-project/ray/blob/master/doc/source/train/getting-started-pytorch.rst
...
5. Launch a distributed training job with a ~ray.train.torch.TorchTrainer class.
...
3. ~ray.train.torch.TorchTrainer launches the distributed training job.
...
1. Adds a ~torch.utils.data.distributed.DistributedSampler to your ~torch.utils.data.DataLoader.
...
> **Tip:** Keep in mind that `DataLoader` takes in a `batch_size` which is the batch size for each worker.
...
> **Note:** If you already manually set up your `DataLoader` with a `DistributedSampler`,
~ray.train.torch.prepare_data_loader will not add another one, and will
respect the configuration of the existing sampler.

> **Note:** ~torch.utils.data.distributed.DistributedSampler does not work with a
`DataLoader` that wraps ~torch.utils.data.IterableDataset.
...
provides performant streaming data ingestion for large scale datasets.
    ...
    See data-ingest-torch for more details.
...
For more details, see train-monitoring-and-logging and train-checkpointing.
