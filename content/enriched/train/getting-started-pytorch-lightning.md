---
id: train.getting-started-pytorch-lightning
title: Get Started with Distributed Training using PyTorch Lightning
topic_type: task
description: ''
subjects:
- train
...
- worker
references: []
canonical_path: /en/latest/train/getting-started-pytorch-lightning
source: https://github.com/ray-project/ray/blob/master/doc/source/train/getting-started-pytorch-lightning.rst
...
4. Launch a distributed training job with a ~ray.train.torch.TorchTrainer.
...
Ray Train also provides a ~ray.train.lightning.RayLightningEnvironment class
...
You can also configure asynchronous checkpointing and
asynchronous validation through the callback.
...
~ray.train.lightning.prepare_trainer to validate
your configurations.
...
> **Note:** If you are using Lightning 2.x, please use the import path `lightning.pytorch.xxx` instead of `pytorch_lightning.xxx`.
...
Ray 2.7 introduced the newly unified ~ray.train.torch.TorchTrainer API, which offers
